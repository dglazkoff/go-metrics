...
func Router(store *storage.MemStorage, cfg *config.Config) chi.Router {
	r := chi.NewRouter()

	r.Post("/update/", logger.Log.Request(gzip.GzipHandle(updateMetricValueInBody(store, cfg))))
	r.Post("/update/{metricType}/{metricName}/{metricValue}", logger.Log.Request(gzip.GzipHandle(updateMetricValueInRequest(store, cfg))))
